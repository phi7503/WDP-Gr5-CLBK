...
const PaymentModal = ({ 
  ...
  // Helper function to get popcorn image - sử dụng ảnh từ database (combo.image)
  const getPopcornImage = (combo) => {
    // ✅ Ưu tiên sử dụng ảnh từ database (combo.image)
    if (combo.image) {
      if (combo.image.startsWith('http://') || combo.image.startsWith('https://')) {
        return combo.image;
      }
      // Nếu là path tương đối, thêm BACKEND_URL
      return `${BACKEND_URL}/${combo.image}`;
    }
    
    // ✅ Fallback: ảnh mặc định nếu không có ảnh trong database
    return 'https://images.unsplash.com/photo-1534939561126-855b8675edd7?w=600&h=400&fit=crop&q=90';
  };
              ...
              <Card 
                ...
                cover={
                  <img
                    ...
                    onError={(e) => {
                      // Fallback to default yellow bucket popcorn image if original fails
                      e.target.src = 'https://images.unsplash.com/photo-1534939561126-855b8675edd7?w=600&h=400&fit=crop&q=90';
                    }}
                  />
                }
              >
