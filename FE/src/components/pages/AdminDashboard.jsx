...
import { TrendingUp, Ticket, ShoppingCart, Filter, ChevronDown, FileDown, FileText } from "lucide-react"
...
export default function AdminDashboard() {
  ...
  queryFn: async () => {
    const params = new URLSearchParams({
      ...
      ...(appliedFilters.selectedDate && { date: appliedFilters.selectedDate }),
      ...(appliedFilters.customRange.from && { from: appliedFilters.customRange.from }),
      ...(appliedFilters.customRange.to && { to: appliedFilters.customRange.to }),
      ...(appliedFilters.selectedMovie !== "all" && { movieId: appliedFilters.selectedMovie }),
      ...(appliedFilters.selectedBranch !== "all" && { branchId: appliedFilters.selectedBranch }),
    });
    const res = await fetch(`http://localhost:5000/api/admin-dashboard/stats?${params}`, {
      credentials: "include",
    });
  ...
  const getMetricConfig = () => {
    const configs = {
      ...
      tickets: { label: "Số vé", color: "#f59e0b", format: (val) => val.toLocaleString() },
      bookings: { label: "Số đơn", color: "#10b981", format: (val) => val.toLocaleString() },
    }
  ...
  const exportToPDF = () => {
    ...
    printWindow.document.write(".summary { margin: 20px 0; padding: 15px; background-color: #f3f4f6; }")
    printWindow.document.write(".summary-item { margin: 10px 0; }")
    printWindow.document.write("</style>")
    printWindow.document.write("</head><body>")
    printWindow.document.write("<h1>Báo cáo Admin Dashboard</h1>")
    printWindow.document.write(`<p>Ngày xuất: ${new Date().toLocaleDateString("vi-VN")}</p>`)

    printWindow.document.write('<div class="summary">')
    printWindow.document.write("<h2>Tổng quan</h2>")
    printWindow.document.write(
      `<div class="summary-item"><strong>Tổng doanh thu:</strong> ${formatVND(data.totalRevenue)}</div>`,
    )
    printWindow.document.write(
    ...
    printWindow.document.write("<thead><tr><th>Ngày</th><th>Doanh thu</th><th>Số vé</th><th>Số đơn</th></tr></thead>")
    printWindow.document.write("<tbody>")

    data.dailyStats.forEach((stat) => {
      printWindow.document.write("<tr>")
      printWindow.document.write(`<td>${new Date(stat.date).toLocaleDateString("vi-VN")}</td>`)
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="bg-gray-900 rounded-lg p-6 mb-6 border border-gray-800">
          <div className="flex items-center justify-between mb-4">
            ...
            <button onClick={() => setShowFilters(!showFilters)} className="lg:hidden text-gray-400 hover:text-white">
          ...
          <div className={`space-y-6 ${showFilters ? "block" : "hidden lg:block"}`}>
            ...
            <div className="border border-gray-800 rounded-lg p-4">
              <h3 className="text-sm font-semibold text-gray-300 mb-3">Thời gian</h3>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                {/* Period Selector */}
                <div>
                  <label className="block text-sm text-gray-400 mb-2">Khoảng thời gian</label>
                  <div className="flex gap-2">
                    <button
                      onClick={() => setPeriod("week")}
                      className={`flex-1 px-4 py-2 rounded-lg font-medium transition-colors ${
                        period === "week" ? "bg-red-600 text-white" : "bg-gray-800 text-gray-400 hover:bg-gray-700"
                      }`}
                    >
                      Tuần
                    </button>
                    <button
                      ...
                      className={`flex-1 px-4 py-2 rounded-lg font-medium transition-colors ${
                        period === "month" ? "bg-red-600 text-white" : "bg-gray-800 text-gray-400 hover:bg-gray-700"
                      }`}
                    >
                      Tháng
                    </button>
                ...
                <div className="md:col-span-2">
                  <label className="block text-sm text-gray-400 mb-2">Khoảng tùy chỉnh</label>
                  <div className="flex gap-2">
                    <input
                      type="date"
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {/* Movie Filter */}
              <div className="border border-gray-800 rounded-lg p-4">
                <h3 className="text-sm font-semibold text-gray-300 mb-3">Phim</h3>
                <select
                  value={selectedMovie}
              ...
              {/* Branch Filter */}
              <div className="border border-gray-800 rounded-lg p-4">
                <h3 className="text-sm font-semibold text-gray-300 mb-3">Chi nhánh</h3>
                <select
          ...
          <div className={`flex gap-3 mt-4 ${showFilters ? "flex" : "hidden lg:flex"}`}>
            <button
        ...
        {isError && (
          <div className="bg-red-900/20 border border-red-600 rounded-lg p-6 text-center">
            ...
            <p className="text-gray-400">{error?.message || "Không thể tải dữ liệu dashboard"}</p>
          </div>
        )}

        {/* Empty State */}
        {!isLoading && !isError && (!data?.dailyStats || data.dailyStats.length === 0) && (
          <div className="bg-gray-900 border border-gray-800 rounded-lg p-12 text-center">
            <div className="text-gray-600 text-6xl mb-4">📊</div>
            <h3 className="text-xl font-semibold text-gray-400 mb-2">Không có dữ liệu</h3>
            <p className="text-gray-500">Không tìm thấy dữ liệu cho khoảng thời gian đã chọn</p>
          </div>
        )}

        {/* Dashboard Content */}
        {!isLoading && !isError && data && data.dailyStats && data.dailyStats.length > 0 && (
          <>
            {/* KPI Cards */}
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
              ...
              <div className="bg-gradient-to-br from-red-900/50 to-gray-900 rounded-lg p-6 border border-red-800">
                ...
                <p className="text-2xl font-bold text-white">{formatVND(data.totalRevenue)}</p>
              </div>
              ...
              <div className="bg-gradient-to-br from-orange-900/50 to-gray-900 rounded-lg p-6 border border-orange-800">
                ...
                <p className="text-2xl font-bold text-white">{data.totalTickets.toLocaleString()}</p>
              </div>
            ...
            <div className="bg-gray-900 rounded-lg p-6 border border-gray-800">
              ...
              <ResponsiveContainer width="100%" height={400}>
                <LineChart data={getChartData()}>
                  ...
                  />
                  <Legend wrapperStyle={{ color: "#9ca3af" }} formatter={() => metricConfig.label} />
                  <Line
                    type="monotone"
                    dataKey="value"
              ...
              {data.query && (
                <div className="mt-4 pt-4 border-t border-gray-800 text-sm text-gray-400">
                  <p>
                    Hiển thị dữ liệu từ {formatDate(data.query.startDate)} đến {formatDate(data.query.endDate)} (
