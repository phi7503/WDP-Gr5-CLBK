...
import { Layout, Typography, Row, Col, Card, List, Spin, Empty } from 'antd';
              ...
              {branches.map(b => (
                <Col xs={24} md={12} key={b._id}>
                  <Card style={{ background: '#1a1a1a', border: '1px solid #333' }}>
                    ...
                    <Text style={{ color: '#999' }}>{b.location}</Text>
                    <div style={{ marginTop: 12 }}>
                      <Text style={{ color: '#fff' }}>Upcoming showtimes</Text>
                      <List
                        size="small"
                        dataSource={(showtimesByBranch[b._id] || []).slice(0, 5)}
                        renderItem={st => (
