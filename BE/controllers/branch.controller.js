...
export const createBranch = async (req, res) => {
  ...
  if (!Array.isArray(theaters)) {
    return res
      .status(400)
      .json({ message: "Theaters must be an array of IDs" });
  }
  ...
  try {
    ...
    if (theaters.length > 0) {
      ...
      if (count !== theaters.length) {
        return res
          .status(404)
          .json({ message: "One or more theaters not found" });
      }
    }
    ...
    const populated = await Branch.findById(createdBranch._id).populate(
      "theaters",
      "name seatLayout"
    );
  ...
  try {
    ...
    if (location) {
      ...
      if (location.coordinates) {
        if (typeof location.coordinates.latitude === "number")
          branch.location.coordinates.latitude = location.coordinates.latitude;
    ...
    if (Array.isArray(theaters)) {
      for (const tId of theaters) {
        if (!isValidObjectId(tId)) {
          return res
            .status(400)
            .json({ message: `Invalid theater ID: ${tId}` });
        }
      }
      const count = await Theater.countDocuments({ _id: { $in: theaters } });
      if (count !== theaters.length) {
    ...
    if (operatingHours) {
      ...
      if (operatingHours.close)
        branch.operatingHours.close = operatingHours.close;
    }
    ...
    const populated = await Branch.findById(updatedBranch._id).populate(
      "theaters",
      "name seatLayout"
    );
