...
export const getShowtimeSeats = async (req, res) => {
  ...
  try {
    ...
    // Combine seat info with status
    const seatsWithStatus = seats.map(seat => {
      const status = seatStatuses.find(ss => ss.seat.toString() === seat._id.toString());
      return {
        _id: seat._id,
        row: seat.row,
        number: seat.number,
        type: seat.type,
        price: status ? status.price : getPriceForSeatType(seat.type, showtime.price),
