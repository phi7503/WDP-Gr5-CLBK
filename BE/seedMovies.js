...
/**
 * Script to seed movies into MongoDB database
...
// Movie Schema (inline definition)
...
const moviesData = [
  ...
  },
  {
    ...
    description: "Phần tiếp theo của series Avengers, với sự xuất hiện của nhiều siêu anh hùng mới và cuộc chiến chống lại kẻ thù mạnh nhất từ trước đến nay.",
    duration: 180,
    ...
    poster: "uploads/posters/avengers-secret-wars-2025.jpg",
    status: "coming-soon"
  },
  {
    title: "Black Panther: Wakanda Forever 2",
    description: "Phần tiếp theo của Black Panther, tiếp tục câu chuyện về Wakanda và những người bảo vệ vương quốc này.",
    duration: 160,
    genre: ["Action", "Adventure", "Sci-Fi"],
    ...
    cast: ["Letitia Wright", "Angela Bassett", "Winston Duke"],
    poster: "uploads/posters/black-panther-2-2025.jpg",
    status: "coming-soon"
  }
];
...
const seedMovies = async () => {
  try {
    ...
    if (existingCount > 0) {
      ...
      console.log('⚠️  To avoid duplicates, existing movies will be kept');
      console.log('💡 If you want to replace all movies, delete them first');
    }
    ...
    console.log('\n📊 Summary:');
    console.log(`🎬 Total new movies: ${insertedMovies.length}`);
    ...
    console.log('🚀 You can now check your frontend to see the new movies!');
    
  } catch (error) {
    if (error.code === 11000) {
      console.log('⚠️  Some movies already exist (duplicate key error)');
      console.log('✅ Non-duplicate movies have been added successfully');
    } else {
...
// Main execution
const main = async () => {
  console.log('🎬 Movie Seeder Script');
  console.log('========================');
  
  if (!process.env.MONGO_URI) {
    console.error('❌ MONGO_URI not found in environment variables');
    console.log('💡 Make sure you have a .env file with MONGO_URI');
    process.exit(1);
  }
